...
public class TaskHandlerExecutorRepository extends AbstractExecutorRepository<TaskRequestWrapper, TaskResult, TaskHandler> {
    ...
    private <R> Future<TaskResult<R>> executeAsyncCommand(final long receiveTime, final TaskHandlerExecutor<?, R> command,
    ...
    private <R> CompletableFuture<TaskResult<R>> executeAsyncCommandV2(final long receiveTime, final AsyncTaskHandlerExecutor<?, R> command,
                                                                       final String commandName, final TaskRequestWrapper requestWrapper) {
        if (command == null) {
            throw new UnsupportedOperationException("Invoked unsupported command : " + commandName);
        } else {
    ...
    public CompletableFuture<TaskResult<Object>> executeAsyncCommandV2(final String commandName, String proxyName, final TaskRequestWrapper requestWrapper) throws UnsupportedOperationException {
        final long receiveTime = System.currentTimeMillis();
        ...
        if (executor instanceof AsyncTaskHandlerExecutor) {
            AsyncTaskHandlerExecutor command = (AsyncTaskHandlerExecutor) executor;
            return executeAsyncCommandV2(receiveTime, command, commandName, requestWrapper);
        } else {
            throw new IllegalStateException("executeAsyncCommandV2 should be called only for commands which implement AsyncTaskHandlerExecutor");
        }
    }
    ...
    public <T, S> TaskResult<T> executeCommand(String commandName, TaskRequestWrapper requestWrapper, Decoder<T> decoder) throws UnsupportedOperationException {
        long receiveTime = System.currentTimeMillis();
        TaskHandlerExecutor<S, T> command = (TaskHandlerExecutor) getExecutor(commandName, commandName, requestWrapper, decoder);
        if (command == null) {
            throw new UnsupportedOperationException("Invoked unsupported command : " + commandName);
        } else {
            try {
     ...
     * @throws UnsupportedOperationException if no handler found for command
     */
    public <R> Future<TaskResult<R>> executeAsyncCommand(String commandName, TaskRequestWrapper requestWrapper, Decoder decoder) throws UnsupportedOperationException {
        final long receiveTime = System.currentTimeMillis();
        TaskHandlerExecutor<?, R> command = (TaskHandlerExecutor) getExecutor(commandName, commandName, requestWrapper, decoder);
        return executeAsyncCommand(receiveTime, command, commandName, requestWrapper);
    }

    public CompletableFuture<TaskResult<Object>> executeAsyncCommandV2(String commandName, TaskRequestWrapper requestWrapper, Decoder decoder) throws UnsupportedOperationException {
        final long receiveTime = System.currentTimeMillis();
    ...
    public CompletableFuture<TaskResult<Object>> executeAsyncCommandV2(String commandName, TaskRequestWrapper requestWrapper) throws UnsupportedOperationException {
    ...
    private Executor<TaskRequestWrapper, TaskResult> getTaskHandlerExecutorWithSemaphoreIsolationAndDecoder(TaskRequestWrapper requestWrapper, String refinedCommandName,
        ...
        if (taskHandler instanceof RequestCacheableHystrixTaskHandler) {
            ...
            return new AsyncTaskHandlerExecutor((AsyncHystrixTaskHandler) taskHandler, this.getTaskContext(), refinedCommandName, requestWrapper, maxConcurrentSize, decoder);
        } else {
